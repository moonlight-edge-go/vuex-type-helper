...
interface ComputedMapper<T> {
  <Key extends keyof T, Map extends Record<string, Key>>(map: Map): { [K in keyof Map]: () => any }
...
interface MethodsMapper<T, R> {
  <Key extends keyof T, Map extends Record<string, Key>>(map: Map): { [K in keyof Map]: (payload: any) => R }
